...
class TypoScraper(scrapy.Spider):
    ...
    start_urls = ['http://www.killerrobotsinc.com']
    rootdomains = ['killerrobotsinc.com']
    squatdomains = dnstwist.fuzz_domain(rootdomains[0])
    checkedPages = []
    bogons = []
    ignoresuffixes = ['mov', 'jpg', 'pdf', 'doc', 'zip', 'bin', 'docx']
    ignoredirectories = ['/page/wiki/Bandolier']
    ...
    def extract_root_domain(self, full_url):
        if full_url[0:7] == "mailto:":
            ...
            try:
                mantissa = full_url.split("://")[1].split("/")[0]
                domain = '.'.join(mantissa.split('.')[-2:])
                return domain
            except:
                return None
